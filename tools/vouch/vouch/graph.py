...
class FunctionInfo:
    ...
    code_hash: str = ""  # hash of the code without docstring/comments — value-sensitive change detection
...
# Dispatch-style "domain folder + role file" layout (views.py → HTTP layer, flows.py → orchestration, ...)
PY_ROLE_LAYERS = {"views": "views", "flows": "flows", "service": "service", "models": "models",
                  "enums": "models", "messaging": "messaging", "scheduled": "scheduled"}
...
class ImportGraph:
    ...
    def classify(self, module: str) -> tuple[str | None, str | None]:
        ...
        if layer is None and len(parts) >= 3 and parts[-1] in PY_ROLE_LAYERS:
            layer = PY_ROLE_LAYERS[parts[-1]]  # domain-per-folder convention: <ctx>/views.py, service.py, ...
    ...
    class Visitor(ast.NodeVisitor):
        ...
        def _func(self, node: ast.FunctionDef | ast.AsyncFunctionDef) -> None:
            ...
            try:
                ...
                clone.body = body_nodes if body_nodes else clone.body
                code_hash = _hashlib.sha1(ast.unparse(clone).encode()).hexdigest()
