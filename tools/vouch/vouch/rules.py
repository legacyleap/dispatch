...
KINDS = ("forbidden_import", "naming", "ownership", "sensitive_path", "paired_change", "manual")
CHECKED_BY = {"forbidden_import": "layer_violation", "naming": "naming", "ownership": "wrong_context",
              "sensitive_path": "scope_drift", "paired_change": "paired_change", "manual": "not machine-checked"}
...
def _evidence(kind: str, params: dict[str, Any]) -> str:
    ...
    if kind == "paired_change":
        return f"deterministic: a change under {params.get('when_globs', [])} must come with a change under {params.get('requires_globs', [])}"
