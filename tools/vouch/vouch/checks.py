        ...
        for fn in m.functions:
            ...
            if fn.qualname not in base_fns:
                out.append(fn)
                              ...
                              or (fn.code_hash and base_fns[fn.qualname].code_hash != fn.code_hash)):
                out.append(fn)  # body or a value in it changed, not just the docstring or formatting
        return out
...
def check_wrong_context(ctx: PRContext, next_id: Any) -> tuple[list[Finding], list[dict[str, Any]]]:
    ...
    for fc in sorted(ctx.src_files(), key=lambda f: -len(ctx.head_modules[f.path].service_calls)):
        ...
        if ctx.graph.language == "java":
            ...
            base_classes = set(ctx.base_modules[fc.path].classes) if fc.path in ctx.base_modules else set()
            text = " ".join([below_ctx] + [f.qualname.lower() for f in new_fns] + [c.lower() for c in head.classes if c not in base_classes])
            vocab = {k: v for k, v in OWNERSHIP_VOCAB.items() if k in bounded}  # built-in vocabulary only for the contexts it names
...
def check_architect_rules(ctx: PRContext, next_id: Any) -> tuple[list[Finding], list[dict[str, Any]]]:
        ...
        for fc in ctx.src_files():
            ...
            for target in sorted(head.imports - before):
                ...
                if not _match_side(to, resolved, _path_of(ctx, resolved), ctx.graph):
                    continue
                if ar["params"].get("cross_context") and ctx.graph.classify(resolved)[0] == _classify_any(ctx, head)[0]:
                    continue  # same context: allowed; the rule is about reaching into another context
                if resolved in seen_targets:
                    continue  # `from x.service import a` and `import x.service` resolve to the same module
    ...
    for ar in rules_file.active(ctx.arch, "paired_change"):
        p = ar["params"]
        triggers = [fc for fc in ctx.files if any(fnmatch.fnmatch(fc.path, g) for g in p.get("when_globs", []))
                    and not any(fnmatch.fnmatch(fc.path, g) for g in p.get("except_globs", []))]
        paired = [fc for fc in ctx.files if any(fnmatch.fnmatch(fc.path, g) for g in p.get("requires_globs", []))]
        if triggers and not paired:
            first = triggers[0]
            findings.append(_finding(
                next_id(), "layer_violation", "architectural", ar.get("severity", "high"), 0.9, "high",
                f"{ar['title']}: `{Path(first.path).name}` changed without `{p.get('requires_label', 'the paired change')}`",
                (f"This PR changes {', '.join('`' + t.path + '`' for t in triggers[:3])} but touches nothing under "
                 f"{', '.join('`' + g + '`' for g in p.get('requires_globs', []))}. {ar['statement']}"),
                _rule(ctx.arch, ar["id"]), {"path": first.path, "line": min(first.added_lines) if first.added_lines else 1, "symbol": Path(first.path).name},
                [{"hop": 0, "module": first.path, "symbol": Path(first.path).name, "path": first.path,
                  "line": min(first.added_lines) if first.added_lines else 1, "note": "changed here"},
                 {"hop": 1, "module": p.get("requires_label", "paired change"), "symbol": "", "path": ", ".join(p.get("requires_globs", [])),
                  "line": 0, "note": "expected in the same PR, not present"}],
                [f"rule {ar['id']} from {rules_file.RULES_PATH}", f"{len(triggers)} trigger file(s), 0 paired file(s)"],
                p.get("fix", "Add the paired change to this PR, or state in the PR why it is not needed."),
                f"Add the change required by {ar['id']} ({p.get('requires_label', 'paired change')}) for {first.path}.",
                snippet(first.patch, min(first.added_lines) if first.added_lines else 1)))
        elif triggers:
            conformant.append({"check": f"architect:{ar['id']}", "label": ar["title"], "detail": "paired change present", "rule": ar["id"]})
